...
function ContributionGraph({ contribution }: { contribution: ContributionProps }) {
    console.log('contribution ', contribution)
    const { weeks } = contribution.data.user.contributionsCollection.contributionCalendar;

    // const monthPair: { label: string, span: number }[] = []
    ...
    return (
        ...
        <div className='card card-dark w-full mb-6 overflow-x-auto'>
            <div className='flex items-start sticky left-0 mb-6'>
                <h5 className='text-base'>{contribution.data.user.contributionsCollection.contributionCalendar.totalContributions || 0} Contributions in the last year</h5>
                                ...
                                <tr key={dayIndex} className='h-2.5'>
                                    ...
                                    </td>
                                    {
                                        weeks.map((week, weekIndex) => {
                                            ...
                                            if(!day) return ''
                                            console.log('day ', day, dayIndex, weekIndex)
